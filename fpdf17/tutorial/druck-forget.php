...
/*$result = @mysql_query("SELECT * 
												FROM Kunden 
												WHERE date_format(Wartungsdatum,'%Y') < date_format(CURRENT_DATE,'%Y') AND Wartungskunde LIKE 1 
...
$result = @mysql_query("	SELECT 
				...
				WHERE date_format(Kunden.Wartungsdatum,'%Y') < date_format(CURRENT_DATE,'%Y') AND Kunden.Wartungskunde LIKE 1 ORDER BY Kunden.ID asc") or print('DB Fehler '.mysql_error());
while($row = @mysql_fetch_object($result)) {
	// zeigt nur die Kunden an, die nicht dieses Jahr und deren Monat noch nicht vollendet ist
	if($row->Wartungs_Monat <  date('m') AND $row->Wartungs_Jahr <  date('Y')){
		//for($i=1;$i<=80;$i++)
